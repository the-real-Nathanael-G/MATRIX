...
Matrix::Matrix(int rows, int cols, const float* initVals) : rows(rows), cols(cols) {
  data = new float[rows * cols];
...
void Matrix::joinHorizontal(const Matrix &other) {
  if (rows != other.rows) {
...
void Matrix::joinVertical(const Matrix &other) {
  if (cols != other.cols) {
...
Matrix Matrix::getRow(int rowIndex) const {
...
bool Matrix::copyRowInto(int rowIndex, float* buf, int buf_len) const {
  if (rowIndex < 0 || rowIndex >= rows) {
    Serial.println("Error: Row index out of range.");
    return false;
  }
